    ...
    if alpha = zero then beta
        ...
        case (alpha, beta) of
          ...
          | _ =>
            let val (p1, _, _, _) = alpha
                val (p2, _, _, _) = beta
                val x_add_y = [(Rat.one, [(0, 1)]),
                                 (Rat.one, [(1, 1)])] : poly
                val y = [(Rat.one, [(1, 1)])] : poly
                val p1_x_add_y = Algebra.p_subst(p1, 0, x_add_y)
                val p2_y = Algebra.p_subst(p2, 0, y)
                val res = Resultant.biv_resultant p1_x_add_y p2_y
                (* Note: res in Q[y]. We must convert to Q[x]. *)
                val f = mk_univ_in(res, 0)
                (* Now, f has (alpha - beta) as a root. But, we must still
               compute an interval [l, u] s.t. (alpha - beta) is f's only
               root in [l, u]. *)
                val (l, u) =
                    refine_with_op(alpha,
                                   ...
                                   (fn ((l1, u1),
                                        (l2, u2)) =>
                                       (Rat.add(l1, Rat.neg u2),
                                        Rat.add(u1, Rat.neg l2))))
            in mk_real_alg f l u NONE end;
...
fun pow (alpha, n) =
    case n of
      ...
      | 1 => alpha
      | 2 => mult(alpha,alpha)
      | _ => let val q = IntInf.quot(n,2)
                 val r = IntInf.rem(n,2)
                 val m = pow(alpha,q)
             in
