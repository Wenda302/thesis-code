...
section \<open>More lemmas for polynomials\<close>
...
lemma proots_within_union:
  "proots_within p (A \<union> B) = proots_within p A \<union> proots_within p B"
  unfolding roots_within_def by auto

lemma proots_within_diff:
  "proots_within p (A - B) = proots_within p A - proots_within p B"
  unfolding roots_within_def by auto

lemma card_roots_within_Int:
  fixes p q::"'a::factorial_ring_gcd poly"
  assumes "p\<noteq>0" "q\<noteq>0"
  shows " card (proots_within p S) + card(proots_within q S) 
          = card(proots_within (p*q) S) + card (proots_within (gcd p q) S)"
using assms
by (auto simp add:proots_within_times proots_within_gcd card_Un_Int[symmetric])
...
lemma proots_count_empty[simp]:"proots_count p {} = 0"
...
lemma proots_count_disj:
  assumes "A\<inter>B = {}" "p\<noteq>0"
  shows "proots_count p (A \<union> B) = proots_count p A + proots_count p B"
proof -
  have "finite (proots_within p A)" "finite (proots_within p B)" 
      "proots_within p A \<inter> proots_within p B = {}" 
    using assms finite_proots unfolding proots_within_def by auto
  with sum.union_disjoint[OF this] show ?thesis
    unfolding proots_count_def by (auto simp add: proots_within_union)
qed  
...
lemma proots_count_subset:
  assumes "A \<subseteq> B" "p\<noteq>0"
  shows "proots_count p A \<le> proots_count p B"
using assms finite_proots unfolding proots_count_def proots_within_def
...
lemma proots_count_0_iff:
  assumes "p\<noteq>0"
  shows "proots_count p s = 0 \<longleftrightarrow> proots_within p s = {}"
proof
  assume "proots_within p s = {}" 
  then show "proots_count p s = 0" unfolding proots_count_def by auto
next
  assume "proots_count p s = 0" 
  let ?rts = "proots_within p s"
  have False when "?rts \<noteq> {}"
    proof -
      obtain r where "r\<in>?rts" using \<open>?rts \<noteq> {}\<close> by blast
      hence "order r p > 0" 
        using order_root \<open>p\<noteq>0\<close> unfolding roots_within_def by auto
      moreover have "proots_count p s = order r p + (\<Sum>r\<in>?rts - {r}. order r p)"
        using sum.insert_remove[of ?rts "\<lambda>r. order r p" r] \<open>p\<noteq>0\<close> \<open>r\<in>?rts\<close>
        unfolding proots_count_def by (simp add: insert_absorb)
      ultimately have "proots_count p s >0" by auto
      thus False using \<open>proots_count p s=0\<close> by simp
    qed
  then show "proots_within p s = {}" by auto
qed
