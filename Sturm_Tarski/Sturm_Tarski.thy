...
next
  ...
  next
    ...
    hence "lead_coeff (-p) > 0" and "lead_coeff p < 0" unfolding lead_coeff_minus
      using leading_coeff_neq_0[OF `p\<noteq>0`] 
...
proof -
  ...
  moreover hence "poly (q mod p) x \<noteq> 0" using `poly q x\<noteq>0` 
...
next
  ...
  thus ?thesis unfolding jump_poly_def
     using order_smult[OF `c\<noteq>0`] sign_r_pos_smult[OF `c\<noteq>0`, of "q*p" x] `q\<noteq>0` 
     by auto
qed
...
next
  ...
  proof -
    ...
    have "?L=simpL"
      ...
      using order_mult[OF `p1*p2\<noteq>0`]
        sign_r_pos_mult[OF `p1\<noteq>0` `p2\<noteq>0`] sign_r_pos_rec[OF `p1\<noteq>0`] \<open>poly p1 x\<noteq>0\<close>
      by auto
    moreover have "poly p1 x>0 \<Longrightarrow> simpL =?R" 
      unfolding simpL_def jump_poly_def using jump_poly_not_root[OF `poly p1 x\<noteq>0`] 
      by auto
    moreover have "poly p1 x<0 \<Longrightarrow> simpL =?R" 
      unfolding simpL_def jump_poly_def using jump_poly_not_root[OF `poly p1 x\<noteq>0`] 
      by auto
  ...
  proof -
    ...
    have "?L=simpL"
      unfolding simpL_def jump_poly_def  
      using order_mult[OF `p1*p2\<noteq>0`]
        sign_r_pos_mult[OF `p1\<noteq>0` `p2\<noteq>0`] sign_r_pos_rec[OF `p2\<noteq>0`] \<open>poly p2 x\<noteq>0\<close>
      by auto
    moreover have "poly p2 x>0 \<Longrightarrow> simpL =?R" 
      unfolding simpL_def jump_poly_def using jump_poly_not_root[OF `poly p2 x\<noteq>0`] 
      by auto
    moreover have "poly p2 x<0 \<Longrightarrow> simpL =?R" 
      unfolding simpL_def jump_poly_def using jump_poly_not_root[OF `poly p2 x\<noteq>0`] 
      by auto
...
next
  ...
  then show ?thesis unfolding jump_poly_def using `q\<noteq>0` by auto   
qed    
...
lemma cindex_poly_cross:
  ...
  shows "cindex_poly a b 1 p = cross p a b" 
    using `poly p a\<noteq>0` `poly p b\<noteq>0`
  ...
  proof -
    ...
    proof (rule sum.cong,rule refl)
      ...
      next
        ...
        ultimately have "cindex_poly a b 1 p' = cross p' a b"
          ...
          using "1.hyps"[rule_format,of "degree p'" p'] `p'\<noteq>0` `poly p' a\<noteq>0` `poly p' b\<noteq>0` 
          by auto  
    ...
    proof (cases "odd (order max_r p)")
      ...
      proof -
        ...
        ultimately show ?thesis 
          using True `poly p' b\<noteq>0` \<open>max_rp\<noteq>0\<close> \<open>p'\<noteq>0\<close> sign_r_pos_mult[OF `p'\<noteq>0` `max_rp\<noteq>0`]
    ...
    next
      ...
      hence "poly max_rp a > 0" and "poly max_rp b > 0"
        ...
        using `poly max_rp a\<noteq>0` `poly max_rp b \<noteq> 0`  "1.prems"(1-2) `poly p max_r = 0`
        apply (unfold zero_less_power_eq)
        by auto
      moreover have "poly max_rp b > 0"
        unfolding max_rp_def poly_power
        using `poly max_rp b \<noteq> 0` False max_rp_def poly_power 
...
proof -
  ...
  have "?L=cindex_poly a b q' p' + cindex_poly a b p' q'"
    ...
    using cindex_poly_mult[OF `g\<noteq>0`] cindex_poly_mult[OF `g\<noteq>0`]
    by auto
  also have "... = cindex_poly a b 1 (q' * p')"
    using  cindex_poly_inverse_add[OF `coprime p' q'`, of a b] .
  also have "... = cross (p' * q') a b"
    using cindex_poly_cross[OF `a<b`, of "q'*p'"] `p'\<noteq>0` `q'\<noteq>0` 
...
next
  ...
  show ?thesis unfolding cindex_poly_def
    ...
    subgoal using poly_roots_finite[OF `p\<noteq>0`] by auto
...
next
  ...
  show ?thesis 
    ...
    by (rule sum.cong,auto simp add:jump_poly_sgn[OF `p\<noteq>0`]) 
qed  
...
proof (induct "smods p q" arbitrary:p q rule:length_induct)
  ...
  proof -     
    ...
    then obtain ps where ps:"smods p q=p#q#r1#ps" "smods r1 r2=r1#ps"
      by (metis \<open>p\<noteq>0\<close> `q \<noteq> 0` r1_def r2_def smods.simps)
    ...
    proof -
      ...
      moreover have "poly p a'\<noteq>0" and "poly q a'\<noteq>0" and "poly r1 a'\<noteq>0" 
        using "1.prems"(2)[unfolded ps] `a\<noteq>a'` by auto
...
proof -
  have "changes_poly_at (smods p q) a = changes_poly_at (smods p q) a'"
    apply (rule changes_smods_congr[OF order.strict_implies_not_eq[OF `a<a'`] `poly p a\<noteq>0`])
