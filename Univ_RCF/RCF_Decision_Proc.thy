...
datatype alg_float = Arep "int poly" float float  
                   | Flt float -- \<open>a small optimization when the number is dyadic\<close>
...
apply (auto simp add:sorted_Cons less_imp_le)
by (metis last_in_set less_imp_le order_trans)
...
lemma sorted_last:"sorted (x#xs) \<Longrightarrow> x \<le> last (x#xs)"
  ...
  apply simp
by (metis last.simps list.distinct(2) order_trans sorted_many_eq)

lemma union_decomp_tl: "sorted (x#xs) \<Longrightarrow> \<Union>set(tl(decomp (x#xs))) = {x..}"
  apply (cases xs)
  by (auto simp add:decomp_def decomp_aux dest:sorted_last)
...
next
  ...
  moreover have "xs\<noteq>[] \<Longrightarrow> ?case"
    proof -
      ...
      def decomps\<equiv>"set (tl (decomp (x2 # xs')))"
      have "strict_sorted xs" and "x1<x2" using Cons.prems unfolding xs by auto
      hence union:"\<Union> decomps = {x2..}" 
        unfolding decomps_def xs
        using union_decomp_tl strict_sorted_sorted by auto
...
next
  ...
  def g\<equiv>"\<lambda>s. if s=y then x else f s"
...
proof -
  ...
  have "ordered_reps (mk_samples root_reps)"
    ...
    next
      ...
      def ys\<equiv>"tl (tl (mk_samples (x2 # xs)))"
  ...
  moreover have "xs\<noteq>[] \<Longrightarrow> p\<noteq>0 \<Longrightarrow> ?case"
    proof -
      ...
      have "\<forall>d\<in>set (tl (decomp xs)). \<forall>y1\<in>d. \<forall>y2\<in>d. sgn_at p y1 = sgn_at p y2"
        ...
        next
          case True
          def x1_p\<equiv>"[:-x1,1:]^(order x1 p)"
          ...
          moreover have "\<forall>d\<in>set (tl (decomp xs)). \<forall>y1\<in>d. \<forall>y2\<in>d. sgn_at x1_p y1 = sgn_at x1_p y2"
            proof (rule+)
              ...
              have "\<Union>set (tl (decomp (x2 # xs'))) = {x2..}" 
                using Cons.prems(2) union_decomp_tl strict_sorted_sorted unfolding xs by auto
...
proof 
  ...
  def pol'\<equiv>"of_float_poly pol"
  ...
  have card_eq:"card {r\<in>set points. poly pol' r =0} = card {x. poly pol' x=0} " 
    proof -
      def sgn_at_pol\<equiv>"\<lambda>p lb ub.  sgn_at_core pol (of_int_poly p) lb ub"
...
proof -
  def decomps\<equiv>"decomp (map of_alg_float root_reps)" 
  def P\<equiv>"\<lambda>x. norm_form2_interp (QF qf_form) (x # vs)"
  def val\<equiv>"\<lambda>x. case x of Arep p lb ub \<Rightarrow> valid_alg p lb ub | Flt x \<Rightarrow> True"
...
ML \<open>
  val rcf_holds = @{computation_check terms:
      ...
      (*float*)
      "op +::float\<Rightarrow>_"
      "op -::float\<Rightarrow>_"
      "op *::float\<Rightarrow>_"
      "uminus::float\<Rightarrow>_"
      "op =::float \<Rightarrow> float \<Rightarrow> bool"

      (*alg_float*)
       "op =::alg_float\<Rightarrow>alg_float\<Rightarrow>bool"
...
method_setup univ_rcf_cert = {*
     Args.term  >>
      (fn cert => fn ctxt => SIMPLE_METHOD' (Univ_RCF.univ_rcf_cert ctxt cert))
*}

(*To use univ_rcf, MATH_KERNEL should point to an executable of Mathematica Kernel*)
method_setup univ_rcf = {*
     Scan.succeed 
...
(*
lemma "let rs = alg_rat_list_conv
              [RAT (- 2), ALG [:- 2, 0, 1:] (- 3 / 2) (- 1), ALG [:- 2, 0, 1:] 1 2];
        fm = qf_form2.Disj (qf_form2.Pos (norm_num2.Pol (int_poly [:- 2, 0, 1:]) 0))
              (qf_form2.Pos (norm_num2.Pol (int_poly [:3, 3 / 2:]) 0))
    in ordered_reps rs \<and>
       contain_all_roots rs
        {of_int_poly (int_poly [:3, 3 / 2:]), of_int_poly (int_poly [:- 2, 0, 1:])} \<and>
       valid_list rs \<and> var_bound fm (Suc 0) \<and> check_qf_form_samples (mk_samples rs) fm "
  unfolding Let_def
apply (tactic {*
    (CONVERSION (rcf_holds @{context}) 1 )
  THEN 
    resolve0_tac [TrueI] 1
*})
  oops

lemma "\<forall>x::real. x^2>2 \<or> 3/2 * x +3>0"
  apply (tactic \<open>
...
lemma "\<exists>x::real. 4*x-2=0"
  ...
  (   (Reflection.default_reflection_tac 
    ...
    THEN' 
        (simp_tac (@{context} delsimps @{thms norm_form2_interp.simps} 
        addsimps @{thms power_numeral_even power_numeral_odd}))
    THEN'
        resolve0_tac [(infer_instantiate' @{context}    
    [NONE,SOME (@{term "[RAT (2/4)]"} |> HOLogic.dest_list |> hd |> 
        (fn x => Thm.cterm_of @{context} (@{term alg_rat_aux_conv} $ x)))] 
    @{thm ExQ_intro_code})] )  1
\<close>)
  
  apply (tactic {*Local_Defs.unfold_tac @{context} [@{thm Let_def}]*})
  apply (tactic {*CONVERSION (rcf_holds @{context}) 1*})
  apply eval
...
oops  
  ...
  by (univ_rcf_cert "[ALG [:- 2, 0, 1:] (1 / 2) (3 / 2)]")
*)  
