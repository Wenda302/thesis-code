...
subsection \<open>Some extra useful lemmas about map_poly\<close>
...
subsection \<open> @{typ "rat poly"} to @{typ "int poly"} by clearing the denominators\<close>
...
next
  ...
  proof (cases "x \<in> set (coeffs p)")    
    ...
    show ?thesis using pCons(2)[OF True]
      by (metis Ints_mult Ints_of_int a2_def de_lcm_pCons lcm_times1 of_int_simps(4) 
